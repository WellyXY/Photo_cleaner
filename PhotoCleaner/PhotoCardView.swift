...
struct PhotoCardView: View {
    ...
    private func loadImage() {
        // 使用强制重载机制加载图片，绕过缓存问题
        photoManager.forceReloadImage(for: photo, size: self.size) { image in
            if let image = image {
                withAnimation {
                    self.image = image
            ...
            if self.photoManager.filteredPhotos.count > 1 {
                let index = self.photoManager.filteredPhotos.firstIndex { $0.id == photo.id } ?? 0
                ...
                if nextIndex < self.photoManager.filteredPhotos.count {
                    ...
                    // 预加载下一张
                    self.photoManager.loadImage(for: nextPhoto, size: self.size) { _ in }
                }
            }
        }
    }
    ...
    private func loadVideo() {
        ...
        photoManager.loadThumbnailForVideo(photo, size: self.size) { image in
            if let image = image {
                withAnimation {
                    self.image = image
